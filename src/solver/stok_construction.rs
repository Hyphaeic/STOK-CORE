...
use crate::types::{StokError, STOKDimensions};
use crate::solver::bellman::{get_policy_transition, gather_by_policy};
...
mod tests {
    ...
    fn test_eta_plus_boundary() {
        ...
        assert!(approx_eq(eta_plus_data[8], 1.0, 1e-5), 
            "Goal state η⁺[2,2,0] should be 1.0, got {}", eta_plus_data[8]);
    }
    ...
    fn test_constrained_stok() {
        ...
        assert!(approx_eq(kappa_data[2], 1.0, 1e-5), "State 2 (goal) should be feasible");
    }
